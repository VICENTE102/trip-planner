...
// `id` maps to a fixed thumbnail in constants/blockImages.ts. Morning and
// afternoon activities reuse the same small set of ids (by theme, not by
// tier) so there are only a handful of distinct thumbnails to source.
...
const NIGHT_ACTIVITIES: Record<TierLevel, TaggedOption[]> = {
  barato: [
    ...
    { id: "general", text: "Cena informal y paseo por el centro", tags: [] },
  ],
  medio: [
    ...
    { id: "general", text: "Cena y paseo por la zona más animada", tags: [] },
  ],
  caro: [
    ...
    { id: "general", text: "Cena de autor y paseo por el distrito más exclusivo", tags: [] },
  ],
};
...
export function pickMorningActivity(
  ...
  const option = pickByPreference(MORNING_ACTIVITIES[tier], preferences, random);
...
export function pickAfternoonActivity(
  ...
  const option = pickByPreference(AFTERNOON_ACTIVITIES[tier], preferences, random);
  return { id: option.id, text: option.text.replace("{destination}", destination) };
}
...
export function pickNightActivity(
  ...
  return pickByPreference(NIGHT_ACTIVITIES[tier], preferences, random).text;
}
